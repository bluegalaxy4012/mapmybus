...
def init_mongo():
    ...
    for agency_id in AGENCY_IDS:
        ...
        routes_df = pd.read_json(get_routes_file(agency_id), dtype={
            'route_id': int,
        ...
        print(f"Inserted routes for agency {agency_id}, count: {routes_collection.count_documents({})}")


        stops_df = pd.read_json(get_stops_file(agency_id), dtype={
            'stop_id': int,
            'stop_lat': float,
            'stop_lon': float,
            'stop_name': str
        ...
        print(f"Inserted stops for agency {agency_id}, count: {stops_collection.count_documents({})}")


        trip_stops_df = pd.read_json(get_trip_stops_file(agency_id), dtype={
            'trip_id': str,
            'stop_id': int,
            'stop_sequence': int
        })
        ...
        print(f"Inserted trip stops for agency {agency_id}, count: {trip_stops_collection.count_documents({})}")

        shapes_df = pd.read_json(get_shapes_file(agency_id), dtype={
            'shape_id': str,
            'shape_pt_lat': float,
            'shape_pt_lon': float,
        ...
        print(f"Inserted shapes for agency {agency_id}, count: {shapes_collection.count_documents({})}")
